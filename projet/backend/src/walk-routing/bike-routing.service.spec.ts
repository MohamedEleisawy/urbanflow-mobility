...
// =============================================================================
// Routage vélo — contrat du service (miroir du piéton)
// =============================================================================
...
// demander un profil automobile ni emprunter l'adresse du piéton.
// =============================================================================
...
describe('BikeRoutingService', () => {
  let service: BikeRoutingService;
  ...
  beforeEach(() => {
    process.env.BIKE_ROUTING_PROVIDER = 'valhalla';
    process.env.BIKE_ROUTING_BASE_URL = 'https://valhalla.test';

    fetchSimule = jest.fn();
    global.fetch = fetchSimule;

    service = new BikeRoutingService();
  });

  afterEach(() => {
    delete process.env.BIKE_ROUTING_PROVIDER;
    delete process.env.BIKE_ROUTING_BASE_URL;
  ...
  describe('configuration', () => {
    ...
    it('N’EMPRUNTE PAS l’adresse du routeur piéton', async () => {
      // ⚠️ Aucun repli caché : un `BIKE_ROUTING_*` absent laisse le vélo sans
      // moteur, même si `WALK_ROUTING_BASE_URL` est renseigné.
      delete process.env.BIKE_ROUTING_PROVIDER;
      delete process.env.BIKE_ROUTING_BASE_URL;
  ...
  describe('appel au moteur', () => {
    it('demande le profil VÉLO, jamais un profil piéton ni automobile', async () => {
      // ⚠️ LE TEST LE PLUS IMPORTANT DE CE FICHIER. Un moteur voiture rendrait
      // un itinéraire pour un autre véhicule — mauvaise vitesse, mauvais
      // interdits. Bien plus trompeur qu'une droite assumée.
      ...
      expect(envoye.costing).toBe('bicycle');
    });
