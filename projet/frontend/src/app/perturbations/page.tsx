...
import { capacites, type CapaciteRoutage } from "@/lib/capacites-api";
...
export default function PerturbationsPage() {
  ...
  /**
   * L'état de la source TEMPS RÉEL, ou `null` tant qu'on l'ignore.
   *
   * ⚠️ SANS CETTE INFORMATION, LA PAGE MENT PAR OMISSION. « Aucune
   * perturbation en cours » se lit comme « le réseau circule normalement ».
   * Or si aucun flux temps réel n'est branché, la phrase exacte est « nous ne
   * savons rien » — ce qui appelle un tout autre comportement de la part de
   * quelqu'un qui part prendre son tram.
   */
  const [tempsReel, setTempsReel] = useState<CapaciteRoutage | null>(null);

  useEffect(() => {
    const controleur = new AbortController();

    capacites(controleur.signal)
      .then((reponse) => setTempsReel(reponse.transitRealtime))
      .catch(() => {
        // ⚠️ SILENCIEUX, MAIS PAS OPTIMISTE : `tempsReel` reste `null`, et
        // l'encadré ci-dessous s'affiche alors comme si rien n'était
        // configuré. Un échec réseau ne doit jamais faire croire à une source
        // branchée.
      });
    ...
    <Container>
      <section className="py-10 sm:py-14">
        ...
        <div className="mt-4 max-w-2xl">
          <EtatSourceTempsReel capacite={tempsReel} />
        </div>
        ...
        */}
        ...
        <div className="mt-8">
          {erreur ? (
            ...
            // ⚠️ LA DESCRIPTION DÉPEND DE LA SOURCE. « Le réseau circule
            // normalement » est une AFFIRMATION : elle n'est défendable que
            // si quelque chose l'a vérifiée. Sans temps réel, la phrase
            // honnête est « aucune perturbation ne nous a été signalée ».
            <EmptyState
              title="Aucune perturbation signalée"
              description={
                tempsReel?.status === "CONFIGURED"
                  ? "Le réseau circule normalement, d'après les dernières informations publiées par les opérateurs."
                  : "Aucune perturbation ne nous a été signalée. Faute de source temps réel, cela ne garantit pas que le réseau circule normalement."
              }
            />
          ) : (
...
function annonce(reponse: AlertsResponse | null): string {
  ...
  if (reponse.items.length === 0) {
    return "Aucune perturbation signalée sur le réseau.";
  }
...
/**
 * Ce que l'application sait — ou ne sait pas — du temps réel.
 *
 * ═══ POURQUOI CET ENCADRÉ EST INDISPENSABLE ═══
 *
 * Une page de perturbations vide se lit « tout va bien ». C'est vrai quand une
 * source temps réel répond et ne signale rien ; c'est faux quand aucune source
 * n'est branchée. Les deux situations produisent le MÊME écran, et seule cette
 * phrase les distingue.
 *
 * ⚠️ AFFICHÉ MÊME QUAND LA SOURCE EST CONFIGURÉE, sous une autre forme. Une
 * information qui n'apparaît que dans le mauvais cas apprend à l'usager à ne
 * pas la chercher.
 */
function EtatSourceTempsReel({ capacite }: { capacite: CapaciteRoutage | null }) {
  // ⚠️ `null` — pas encore chargé, ou échec — EST TRAITÉ COMME NON CONFIGURÉ.
  ...
  const configure = capacite?.status === "CONFIGURED";

  if (configure) {
    return (
      <p className="border-eco/30 bg-eco/5 text-eco rounded-md border px-4 py-3 text-sm">
        <span className="font-semibold">Temps réel actif</span> — source{" "}
        {capacite?.provider}. Retards et suppressions sont pris en compte.
      </p>
    );
  }
  ...
  return (
    <p className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900">
      <span className="font-semibold">
        Les informations temps réel de transport ne sont pas configurées pour
        ce territoire.
      </span>{" "}
      Cette page n&apos;affiche que les perturbations déjà publiées et
      importées&nbsp;: un incident survenu il y a quelques minutes n&apos;y
      figure pas. La CTS diffuse ces informations au format SIRI-Lite, sous
      jeton nominatif, absent de cette installation.
    </p>
  );
}
